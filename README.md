# DgsGraphstream UI Setup Manual
...
## Step 5: Installing ```dgs-graphstream-ui```

### 5.1 Clone the repository
[dgs-graphstream-ui](https://github.com/cmmin/dgs-graphstream-ui.git) can be cloned from github:

```bash
>> cd /path/to/graphstream
>> git clone https://github.com/cmmin/dgs-graphstream-ui.git
```
...
1. ```dgs-graphstream-ui``` needs to know the full paths to ```dgs-graphstream```, ```ffmpeg```, ```montage``` and the full paths to the folders that contain the executables for```OSLOM2```, ```gvmap``` & ```infomap```.
...
2. ```dgs-graphstream-ui``` checks for all the available ```python3``` packages and warns whether any still need to be installed (install via: ```pip3 install <packagename>``` and then relaunch ```dgs-graphstream-ui```)

## Step 6: Creating and loading Projects

### 6.1 Creating an Empty new Project

When ```dgs-graphstream-ui``` launches, select "Create New Project".
...
When ```dgs-graphstream-ui``` launches, select "Load Existing Project".
...
1. Create an empty new project in a new folder located at ```path/to/dgs-graphstream-ui/examples/```
...
3. Use the ```dgs-graphstream-ui``` program to setup the full example parameters
...
Open ```path/to/dgs-graphstream-ui/src/qml/assets/uitext.csv``` in a plain text editor. The file is organised with comma separated values. 
